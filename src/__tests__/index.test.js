...
describe('writer', () => {
  ...
  it('links references in subject', async () => {
    ...
    const result = writerOpts.transform(commit, createContext());
    expect(result.subject).toBe('Change something ([#3](https://github.com/my-user/my-module/issues/3))');
  });
  ...
  it('links reference actions in subject', async () => {
    ...
    const result = writerOpts.transform(commit, createContext());
    expect(result.subject).toBe('Change something, close [#3](https://github.com/my-user/my-module/issues/3)');
